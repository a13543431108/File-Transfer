...
class Node(threading.Thread):
    ...
    def _send_folder(self, sock, folder_path, target_ip):
        ...
        if ack == b'NO':
            self.gui.log("[发送] 对方拒绝传输（文件夹已存在且不覆盖）")
            return
        if ack != b'OK':
            self.gui.log("[发送] 对方拒绝传输")
            return
            ...
            try:
                with open(filepath, 'rb') as f:
                    ...
                    while sent < file_size:
                        ...
                        sent += n
                        if progress_cb:
                            progress_cb(sent, file_size, time.time())
                    return
            except Exception as e:
                self.gui.log(f"[发送] sendfile 失败，回退到普通发送: {e}")
        # 普通发送：双缓冲预读 + 自适应块大小（借鉴下载器设计）
        sent = offset
        with open(filepath, 'rb', buffering=BUFFER_SIZE) as f:
            if offset:
                f.seek(offset)
            ...
            ACTIVITY_TIMEOUT = 30
            LOW_NETWORK_THRESHOLD = 512 * 1024
            # 单缓冲发送（避免双缓冲占用过多内存，8MB 已足够）
            buf = bytearray(BUFFER_SIZE)
            avg_speed = float('inf')  # 初始化为无穷，避免首次循环误触发超时
            while sent < file_size:
                # 读取一块
                chunk_size = min(BUFFER_SIZE, file_size - sent)
                f.readinto(memoryview(buf)[:chunk_size])
                # 发送当前块
                sock.sendall(memoryview(buf)[:chunk_size])
                sent += chunk_size
                # 自适应调整块大小（借鉴下载器速度采样）
                now = time.time()
                ...
                if remaining > 0:
                    next_read = min(adaptive_chunk_size, remaining)
                    # 单缓冲：直接读入 buf 开头，下次循环会覆盖
                    f.readinto(memoryview(buf)[:next_read])
                if progress_cb:
                    progress_cb(sent, file_size, time.time())
    ...
    def _send_single_file(self, sock, filepath, target_ip):
        ...
        ack = recv_exact(sock, 2)
        if ack == b'NO':
            self.gui.log("[发送] 对方拒绝传输（文件已存在且不覆盖）")
            return
        if ack != b'OK':
            self.gui.log("[发送] 对方拒绝传输")
            return
